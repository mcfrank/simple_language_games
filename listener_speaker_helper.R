...
return.sorted.bets <- function(data,speaker) {
    v1 = array(dim=3)
    v2 = array(dim=3)
    sdata <- data.frame(WorkerId=data$WorkerId,
                     pred1=NA,pred2=NA,pred3=NA,
                     spred1=NA,spred2=NA,spred3=NA,
    ...
    for (i in 1:length(data$HITId)) {
        ...
        like.2 <- get.listener.speaker.preds(v1,v2,speaker)
        ...
        sdata[i,c("spred1","spred2","spred3")] <- sorted.like2
...
## ----------------------------------------------------------------------
## get predictions for the listener using the speaker
get.listener.speaker.preds <- function(v1,v2,speaker) {
	like <- c(NA,NA,NA)
	for (j in 1:3) {
		n1 <- sum(v1==v1[j])
		n2 <- sum(v2==v2[j])
		f <- speaker[[1]]$n1==n1 & speaker[[1]]$n2==n2
		
		if (sum(f)==0) {
			f <- speaker[[1]]$n2==n1 & speaker[[1]]$n1==n2
			like[j] <- v1[j] * (1- speaker[[1]]$b1[f])
		} else {
			like[j] <- v1[j] * speaker[[1]]$b1[f]
		}
	}
